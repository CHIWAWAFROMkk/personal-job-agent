...
def run_checks(url: str) -> None:
    ...
    with sync_playwright() as p:
        ...
        for name, width, height in [("desktop", 1440, 1000), ("mobile", 390, 844), ("landscape", 844, 390), ("compact", 1024, 768)]:
            ...
            assert page.locator(".job-choice.is-applied").evaluate("el => getComputedStyle(el).borderTopColor") == "rgb(67, 131, 98)"
            expect(page.locator(".job-group-title").last).to_contain_text("已投递")
            page.screenshot(path=str(OUTPUT / f"{name}-application-groups.png"))
            page.locator('[data-job-filter="applied"]').click()
            expect(page.locator(".job-choice")).to_have_count(1)
            expect(page.locator(".job-choice")).to_have_attribute("data-select-job", "1")
            page.screenshot(path=str(OUTPUT / f"{name}-applied-filter.png"))
