...
class FillBridgeTests(unittest.TestCase):
    def test_session_isolation_expiry_close_and_limit(self):
        now = [0]
        store = FillBridge(clock=lambda: now[0])
        sid = store.create(1, "https://jobs.example/a", "https://jobs.example/b")["session_id"]
        self.assertEqual(store.list(2), [])
        self.assertIsNone(store.poll(sid)["code"])
        store.LIMIT = 1
        with self.assertRaises(ValueError):
            store.create(2, "https://jobs.example/a", "https://jobs.example/b")
        now[0] = 121
